...
create or alter  procedure sp_sap_utils_XMlresponse
    @error nvarchar(255), @message nvarchar(255), @fecha_actual DATETIME = NULL
AS
BEGIN
    SET @fecha_actual = ISNULL(@fecha_actual, GETDATE());
    SELECT @error AS 'error', @message AS 'message', @fecha_actual AS 'fecha_actual' FOR XML PATH(''), ROOT('XMLresponse')
...
BEGIN
    ...
    SELECT [user].user_name, conn.conn_guid, CONVERT(INT, 0) AS [error], @fecha_actual AS 'fecha_actual'
    FROM _sap_users [user]
    JOIN _sap_conn conn ON [user].user_id = conn.user_id
...
/********************************************************************************************************************/
/* sp_sap_add_cart
   Description: Enseña el producto que se va a añadir al carrito.
*/
/********************************************************************************************************************/

create OR ALTER procedure sp_sap_add_cart
    @product nvarchar(255)
AS
BEGIN
DECLARE @ret integer = 1
		BEGIN
         EXEC sp_sap_utils_XMlresponse @ret,@message = @product;
		END
    /********************************* TEST UNITARIO*********************************
         exec sp_sap_add_cart manzanas
    *********************************************************************************/
END
go
